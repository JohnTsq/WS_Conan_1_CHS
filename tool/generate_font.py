...
def get_unique_charset(text_path) -> str:
...
def main():
    jpn_tbl = tbl_to_dict("main_jpn.tbl")
    charset = ''
    ...
    for char in charset:
        if char not in chs_tbl:
            ...
            with open(rf'graphic\font.asm', 'a', encoding='utf-8') as f:
                ...
                codepoint = codepoint.to_bytes(2, 'little').hex()
    ...
    append_start = len(jpn_tbl)
    with open(rf'graphic\font.asm', 'a', encoding='utf-8') as f:
        f.write(f'.orga 0x{24*append_start:06X}\n\t')
        append_start = append_start.to_bytes(2, 'little').hex()
        append_end = (len(chs_tbl)-1).to_bytes(2, 'little').hex()
